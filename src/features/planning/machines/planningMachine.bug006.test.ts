...
describe('BUG-006: Cannot proceed past step 1', () => {
  ...
  it('reproduces the bug: Next button is disabled on step 1 before form submission', () => {
    ...
    const canGoNext = snapshot.context.currentStepNumber < 10 &&
                     snapshot.context.completedSteps.includes(snapshot.context.currentStepNumber);
  ...
  it('correct workflow: Form submission stores responses and triggers artifact generation', async () => {
    ...
    actor.send({
      ...
      responses: {
        ...
        projectDescription: 'I want to build a simple html page that when i click anywhere in the page the background fades to a new random color.',
      },
    });
