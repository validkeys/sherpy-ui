...
describe('BUG-007: Gap Analysis Submit No API Call', () => {
  ...
  it('should trigger artifact generation API call when submit is clicked', async () => {
    ...
    await waitFor(() => {
      const requirementsFieldDisabled = (screen.getByLabelText(/Do you have existing requirements/i) as HTMLInputElement).disabled;
      const descriptionFieldDisabled = (screen.getByLabelText(/What are you building/i) as HTMLTextAreaElement).disabled;
  ...
  it('exposes bug: form data is empty when submit is clicked', async () => {
    ...
    const formDataLogs = consoleLogs.filter(log => log.includes('[FormStep] Updated formData:'));
    ...
    const submitLogs = consoleLogs.filter(log => log.includes('[FormStep] Form data:'));
    ...
    const eventLogs = consoleLogs.filter(log => log.includes('[FormStep] Sending event:'));
  ...
  it('reproduces exact bug scenario from bug report', async () => {
    render(
      <PlanningMachineProvider input={{ ...defaultInput, projectId: 'LcINIWVz' }}>
        <FormStep stepKey="step1_gapAnalysis" stepName="Gap Analysis" status="collecting" />
      </PlanningMachineProvider>
    );
    ...
    await waitFor(() => {
      ...
      const requirementsField = screen.getByLabelText(/Do you have existing requirements/i) as HTMLInputElement;
      const descriptionField = screen.getByLabelText(/What are you building/i) as HTMLTextAreaElement;
      ...
      expect(descriptionField.value).toBe('Healthcare Portal - Patient management system');
    }, { timeout: 1000 });
  });
