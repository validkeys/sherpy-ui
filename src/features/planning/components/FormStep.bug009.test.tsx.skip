...
describe('BUG-009: XState Machine Not Initializing', () => {
  ...
  beforeEach(() => {
    ...
    console.log('[BUG-009 Test] localStorage mocked. Initial length:', Object.keys(mockStorage).length);
  ...
  it('should create localStorage key after mounting PlanningMachineProvider', async () => {
    console.log('[BUG-009 Test] ===== TEST START: localStorage key creation =====');
  ...
  it('should persist state changes when actor receives events', async () => {
    console.log('[BUG-009 Test] ===== TEST START: state persistence on events =====');
    ...
    await waitFor(() => {
      ...
      if (!updatedState) {
        console.error('[BUG-009 Test] ❌ localStorage key deleted after submit');
      ...
      expect(parsed.context.step1Responses.projectDescription).toBe('Test project for BUG-009');
    }, { timeout: 3000 });
  ...
  it('should handle StrictMode double-mounting without breaking persistence', async () => {
    console.log('[BUG-009 Test] ===== TEST START: StrictMode double-mounting =====');
    ...
    await waitFor(() => {
      const keys = Object.keys(mockStorage);
      console.log('[BUG-009 Test] mockStorage keys after StrictMode mount:', keys);

      if (keys.length === 0) {
        console.error('[BUG-009 Test] ❌ StrictMode double-mount broke persistence');
      }
  ...
  it('should expose actor globally for debugging (window.__planningActor)', async () => {
    ...
    await waitFor(() => {
      console.log('[BUG-009 Test] window.__planningActor exists?', !!(window as any).__planningActor);
  ...
  it('REPRODUCTION: exact Test Run #003 scenario - form submit with clean localStorage', async () => {
    console.log('[BUG-009 Test] ===== TEST START: Test Run #003 reproduction =====');
    console.log('[BUG-009 Test] Simulating: localStorage.clear() -> create project -> fill form -> submit');
    ...
    console.log('[BUG-009 Test] Component mounted. Checking initial localStorage...');
    ...
    await waitFor(() => {
      ...
      if (!hasKey) {
        console.error('[BUG-009 Test] ❌ REPRODUCTION SUCCESSFUL: No localStorage key created on mount');
    ...
    console.log('[BUG-009 Test] Submit clicked. Waiting for state persistence...');
    ...
    await waitFor(() => {
      ...
      if (!storedData) {
        console.error('[BUG-009 Test] ❌ CRITICAL: No localStorage after submit');
        console.error('[BUG-009 Test] This is the exact bug from Test Run #003');
    ...
    if (finalState) {
      ...
      if (parsed.context.currentStepNumber === 1) {
        console.warn('[BUG-009 Test] ⚠️ Still on Step 1 after 2s - artifact generation may have failed');
      }
    }
