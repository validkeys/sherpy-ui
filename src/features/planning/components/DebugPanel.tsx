...
export function DebugPanel() {
  ...
  useEffect(() => {
    const subscription = actor.subscribe((snapshot) => {
      ...
      setEventHistory((prev) => [
        ...prev.slice(-9),
        `${timestamp}: State changed to ${JSON.stringify(snapshot.value)}`,
      ]);
    });
  ...
  const captureSnapshot = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/dev/snapshot/capture", {
        method: "POST",
      ...
      setCaptureStatus(`✅ Saved: ${result.filename}`);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: status === "active" ? "#00ff00" : "#ff0000" }}>
          {status}
        </span>
        <br />
      ...
      >
        ...
        <br />
        <strong>Artifacts:</strong> {Object.keys(context.artifacts).length}{" "}
        generated
        <br />
        {context.error && (
          <div style={{ color: "#ff0000", marginTop: "4px" }}>
        ...
        <div style={{ maxHeight: "100px", overflow: "auto", marginTop: "4px" }}>
          {eventHistory.length === 0 ? (
            ...
            eventHistory.map((evt, idx) => (
              <div
                key={idx}
                style={{ fontSize: "10px", marginBottom: "2px", color: "#888" }}
              >
                {evt}
              </div>
            ))
      ...
      >
        ...
        {captureStatus && (
          <div
            style={{
              fontSize: "10px",
              marginTop: "4px",
              color: captureStatus.startsWith("✅")
                ? "#00ff00"
        ...
        <div style={{ fontSize: "10px", color: "#888", marginTop: "4px" }}>
          Saves current machine state to tests/fixtures/snapshots/ for testing
        </div>
      </div>

      {/* Manual Event Sender */}
      <div
        style={{ padding: "8px", background: "#0a0a0a", borderRadius: "4px" }}
      >
...
function ManualEventSender({
  actor,
}: {
  actor: ActorRefFrom<typeof planningMachine>;
}) {
  ...
  return (
    <div style={{ marginTop: "8px" }}>
      ...
      <div style={{ fontSize: "10px", color: "#888", marginTop: "4px" }}>
        This bypasses the form and sends the event directly to test if the
        machine responds
      </div>
    </div>
  );
