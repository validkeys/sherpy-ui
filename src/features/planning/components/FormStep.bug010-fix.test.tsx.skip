...
describe('BUG-010 Fix: DOM value recovery on submit', () => {
  ...
  it('should recover form data from DOM when React state is empty (BUG-010 scenario)', async () => {
    ...
    const requirementsInput = screen.getByLabelText('Do you have existing requirements?') as HTMLInputElement;
    const descriptionTextarea = screen.getByLabelText('What are you building?') as HTMLTextAreaElement;
    ...
    console.log('[BUG-010 FIX TEST] Set DOM values without triggering onChange');
    console.log('[BUG-010 FIX TEST] requirementsInput.value:', requirementsInput.value);
    console.log('[BUG-010 FIX TEST] descriptionTextarea.value:', descriptionTextarea.value);
    ...
    requirementsInput.value = 'No, starting from scratch - modified by autofill';
    descriptionTextarea.value = 'Healthcare portal - modified by autofill';

    console.log('[BUG-010 FIX TEST] Modified DOM values after onChange (simulating autofill override)');
    ...
    await waitFor(() => {
      ...
      expect(responses.existingRequirements).toContain('modified by autofill');
  ...
  it('should handle partial React state (some fields empty, some filled)', async () => {
    ...
    console.log('[BUG-010 FIX TEST] Mixed state: field1 from React, field2 from DOM');
