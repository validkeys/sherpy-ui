...
import { INTERVIEW_QUESTION_SCHEMA, ARTIFACT_RESPONSE_SCHEMA, type InterviewQuestionResponse } from '../planning/response-schemas';
...
describe('Structured Output - JSON Schema Validation', () => {
  ...
  test('INTERVIEW_QUESTION_SCHEMA has descriptions for LLM guidance', () => {
    expect(INTERVIEW_QUESTION_SCHEMA.properties.question.description).toBeTruthy();
    expect(INTERVIEW_QUESTION_SCHEMA.properties.options.description).toBeTruthy();
    expect(INTERVIEW_QUESTION_SCHEMA.properties.isComplete.description).toBeTruthy();
  });
...
describe('Structured Output - Response Parsing', () => {
  test('valid JSON response parsed correctly', () => {
    const jsonResponse = JSON.stringify({
      ...
      options: [
        { letter: 'A', title: 'Option A', body: 'Description A', recommended: true },
        { letter: 'B', title: 'Option B', body: 'Description B', recommended: false },
      ],
      isComplete: false,
    });
  ...
  test('question text is clean without options section', () => {
    const jsonResponse = JSON.stringify({
      ...
      options: [
        { letter: 'A', title: 'Option A', body: 'Description A', recommended: false },
      ],
      isComplete: false,
    });
  ...
  test('options extracted properly from JSON', () => {
    const jsonResponse = JSON.stringify({
      ...
      options: [
        { letter: 'A', title: 'First', body: 'First option', recommended: true },
        { letter: 'B', title: 'Second', body: 'Second option', recommended: false },
        { letter: 'C', title: 'Third', body: 'Third option', recommended: false },
      ],
      isComplete: false,
    });
...
describe('Structured Output - Integration Scenarios', () => {
  test('Step 1 with structured output enabled returns valid JSON', () => {
    ...
    const mockJsonResponse = JSON.stringify({
      ...
      options: [
        { letter: 'A', title: 'Known Problem', body: 'We have a clear problem', recommended: true },
        { letter: 'B', title: 'Exploration', body: 'We are exploring options', recommended: false },
      ],
      isComplete: false,
    });
